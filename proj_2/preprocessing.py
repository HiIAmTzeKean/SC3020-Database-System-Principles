...
class Database:
    ...
    def setup(self) -> None:
        ...
        try:
            admin_conn = psycopg2.connect(dbname="postgres", user="postgres", password="postgres", host="localhost", port=5432)
            admin_conn.autocommit = True
            admin_cursor = admin_conn.cursor()

            admin_cursor.execute("SELECT 1 FROM pg_roles WHERE rolname=%s;", ('group1',))
            ...
            admin_cursor.execute("SELECT 1 FROM pg_database WHERE datname=%s;", ('imdb',))
            if admin_cursor.fetchone() is None:
    ...
    def get_qep(self, query: str) -> str | None:
        ...
        try:
            self.cursor.execute(sql.SQL("EXPLAIN (ANALYZE, FORMAT JSON) {}").format(sql.SQL(query)))
